...
def upload_json_to_postgresql(database_name, username, password, json_file_path, primary_key_column, host='localhost', port='5432'):
    try:
        ...
        # # Handle cases where JSON data is a list (no top-level key)
        # if isinstance(data, list):
        #     json_array = data
        # elif isinstance(data, dict):
        #     # If data is a dictionary, extract the first key and its associated array
        #     array_name = list(data.keys())[0]
        #     json_array = data[array_name]

        #     if not isinstance(json_array, list):
        #         raise ValueError(f"The array '{array_name}' is not in the correct format. It should be an array.")
        # else:
        #     raise ValueError("Invalid JSON format. The root element must be an array or an object containing an array.")
        ...
        # # Handle cases where JSON data is a list or dictionary
        # if isinstance(data, list):
        #     json_array = data
        # elif isinstance(data, dict):
        #     array_name = list(data.keys())[0]
        ...
        #     if isinstance(json_array, dict):
        #         json_array = [json_array]  # Convert dictionary to list
        #     elif not isinstance(json_array, list):
        #         raise ValueError(f"The array '{array_name}' is not in the correct format. It should be an array.")
        # else:
        #     raise ValueError("Invalid JSON format. The root element must be an array or an object containing an array.")
        ...
        for _, row in df.iterrows():
            ...
            #     cur.execute(insert_query, tuple(converted_row))
            if exists:
                print(f"Row with primary key {row[primary_key_column]} already exists. Deleting and updating.")
                
                # Delete the existing row with the matching primary key
                delete_query = sql.SQL("DELETE FROM {} WHERE {} = %s").format(
                    sql.Identifier(table_name),
                    sql.Identifier(primary_key_column)
                )
                cur.execute(delete_query, (row[primary_key_column],))
