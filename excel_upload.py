...
#                     sql.Identifier(primary_key_column)
#                 )
#                 cur.execute(select_query, (primary_key_value,))
#                 db_row = cur.fetchone()

#                 if db_row:
#                     # Map database row to a dictionary for easier comparison
#                     db_row_dict = dict(zip([desc[0] for desc in cur.description], db_row))

#                     # Compare each column value (excluding the primary key)
#                     has_changes = any(
#                         str(db_row_dict[col]) != str(row[col])
#                         for col in df.columns if col != primary_key_column
#                     )

#                     if has_changes:
#                         # Only delete and reinsert if data has changed
#                         delete_query = sql.SQL("DELETE FROM {} WHERE {} = %s").format(
#                             sql.Identifier(table_name),
#                             sql.Identifier(primary_key_column)
#                         )
#                         cur.execute(delete_query, (primary_key_value,))
#                         print(f"Deleted row with {primary_key_column} = {primary_key_value} in table '{table_name}'.")
...
#                         )
#                         cur.execute(insert_query, tuple(row))
#                         print(f"Inserted updated row with {primary_key_column} = {primary_key_value} in table '{table_name}'.")
#                     else:
#                         print(f"No changes for row with {primary_key_column} = {primary_key_value}.")
#                 else:
...
#                     )
#                     cur.execute(insert_query, tuple(row))
#                     print(f"Inserted new row with {primary_key_column} = {primary_key_value} in table '{table_name}'.")
